...
// gemini-3.8-flash가 프리뷰 단계라 "high demand" 5xx가 종종 발생한다.
// cron은 status='pending'인 레코드만 재처리하므로, 여기서 실패로 확정해버리면
// 자동으로 다시 시도될 기회가 없다. 일시적 서버 과부하(5xx)는 짧게 재시도한다.
...
async function callGeminiOnce(apiKey, inputText) {
  ...
  if (response.status === 429) {
    throw new GeminiQuotaExceededError("Gemini 무료 티어 한도 초과 (429)");
  }
